...
def build_generation_prompt(history: str, emotion: str, speaker_name: str) -> List[Dict[str, str]]:
    """Build prompt for emotion-steered response generation"""
    shots = [
        {
            "role": "system",
            "content": (
                "Task: Write the next line of dialogue in a FRIENDS-style TV scene.\n"
                f"The next line should express {emotion} emotion.\n"
                f"• Speaker: {speaker_name}\n"
                "• Keep it SHORT (max 15 words)\n"
                "• Start with speaker name followed by colon"
            )
        }
    ]
    ...
    fewshot_examples = [
        ...
        ("My audition went terribly.\nEMOTION=Sad\nSPEAKER=Joey",
         "Joey: I'm never going to make it as an actor."),
        ("You ate my sandwich?!\nEMOTION=Angry\nSPEAKER=Ross",
         "Ross: That was MY sandwich! MY SANDWICH!"),
        ("I got the promotion!\nEMOTION=Excited\nSPEAKER=Monica",
         "Monica: Oh my god! I'm going to be head chef!"),
        ("What's that smell?\nEMOTION=Disgust\nSPEAKER=Rachel",
         "Rachel: Ugh, that's revolting! Get it away from me!"),
        ("I got you a present.\nEMOTION=Surprise\nSPEAKER=Chandler",
         "Chandler: What?! For me? You never get me presents!"),
        ("Let me explain the project requirements.\nEMOTION=Neutral\nSPEAKER=Monica",
         "Monica: I understand. Just tell me what needs to be done.")
    ]
...
def build_baseline_prompt(history: str, speaker_name: str) -> List[Dict[str, str]]:
    """Build prompt for neutral baseline response generation"""
    shots = [
        {
            "role": "system",
            "content": (
                "Task: Write the next line of dialogue in a FRIENDS-style TV scene.\n"
                "The next line should express a NEUTRAL emotion.\n"
                f"• Speaker: {speaker_name}\n"
                "• Keep it SHORT (max 15 words)"
            )
        }
    ...
    fewshot_examples = [
        ...
        ("The weather is nice today.\nSPEAKER=Monica",
         "Monica: Yes, it's supposed to stay clear all week.")
    ]
